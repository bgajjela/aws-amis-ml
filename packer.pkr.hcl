...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo bash -lc 'source /etc/profile.d/nix.sh && nix build -o /opt/nix/envs/base-py312 /opt/nix/flake#py-base-py312'",
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo bash -lc 'source /etc/profile.d/nix.sh && nix build -o /opt/nix/envs/pro-py312 /opt/nix/flake#py-pro-py312'",
      ...
      "if [ -x /opt/nix/envs/pro-py312/bin/python ]; then sudo ln -sf /opt/nix/envs/pro-py312/bin/python /usr/local/bin/py312; elif ls /opt/nix/envs/pro-py312/bin/python3.* >/dev/null 2>&1; then sudo ln -sf $(ls /opt/nix/envs/pro-py312/bin/python3.* | head -n1) /usr/local/bin/py312; fi",
